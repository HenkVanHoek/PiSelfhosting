...
### 1.6 How the "Access Your Services" Links are Generated

- **Principle**: The "Access Your Services" link list displayed at the end of a successful deployment is not based on static metadata. It is dynamically generated by inspecting the final deployment artifacts to ensure the links are a perfect reflection of what was actually deployed. This adheres to the "Single Source of Truth" principle, where the `deployment_context.json` file is the authority on configuration.

- **Process Flow**: The link generation is handled by `DeploymentManager` and follows this sequence:

  1.  **Identify Deployed Services**: The manager parses the final `docker-compose.yml` file in the output directory to get the exact list of deployed service names.

  2.  **Consult Component Metadata**: For each service, it looks up its definition in `config/components_metadata.json`.

  3.  **UI Gatekeeper Check**: It first checks for the `"has_ui": true` flag in the metadata. If this is not present and true, no link will be generated for that service.

  4.  **Extract Port Variable**: The system inspects the `ports` section of the `docker-compose.yml` file to find the Jinja2 variable name used for the host port mapping (e.g., `PORTAINER_HTTPS_PORT`).

  5.  **Resolve Final Port Value**: It then uses this variable name to look up the final, user-configured port number from the `deployment_context.json` file. This is the most critical step. If the variable is not found in the context, the link generation for that service is silently aborted.

  6.  **Determine Protocol**: The `protocol` (`http` or `https`) is retrieved directly from the component's metadata.

  7.  **Construct URL**: Finally, the link is assembled using the protocol, the target device's IP, and the resolved port number.

- **Troubleshooting**: If a service link is missing, the most likely cause is a failure in step #5. This typically indicates that the variable defined in a component's `variables.json` was not correctly passed through the UI and setup process, and is therefore missing from the final `deployment_context.json`.
