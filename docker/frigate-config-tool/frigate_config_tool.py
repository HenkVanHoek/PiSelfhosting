# /home/PiSelfhosting/scripts/frigate_config_tool.py
...
DASHY_CONFIG_PATH = os.path.join(BASE_DIR, "docker", "dashy", "config", "conf.yml") # Path to Dashy config
            ...
            for port in onvif_ports_to_try:
                ...
                if onvif_service_detected:
                        ...
                        try:
                            ...
                            for profile in profiles:
                                try:
                                    ...
                                    if rtsp_uri:
                                        encoded_user = url_encode_password(cred['user'])
                                        encoded_pass = url_encode_password(cred['pass'])
                    ...
                    if not camera_accessed:
                        ...
                        print("    Tip: Als de camera niet reageert, kan het zijn dat deze een beperkt aantal actieve streams heeft. Controleer andere applicaties die de camera gebruiken.") # Added warning
                            ...
                            try:
                                ...
                                for profile in profiles:
                                    try:
                                        ...
                                        rtsp_uri = uri_response.Uri
                                        if rtsp_uri:
                                            encoded_user = url_encode_password(custom_user_input)
                                            encoded_pass = url_encode_password(custom_pass_input)
                                            if (encoded_user or encoded_pass) and "@" not in rtsp_uri:
                                                rtsp_uri_parts = rtsp_uri.split('//')
                                                auth_string = f"{encoded_user}"
                                                if encoded_pass:
                                ...
                                camera_accessed = True # Mark as accessed with custom creds
                            except (ONVIFError, Fault, asyncio.TimeoutError) as e:
...
def update_dashy_config(new_camera_configs, domain):
    """
    Updates the Dashy conf.yml to add new camera tiles.
    """
    ...
    try:
        ...
        # Find or create the "Smart Home" section
        smart_home_section = next((s for s in dashy_data.get('sections', []) if s.get('name') == 'Smart Home'), None)
        if not smart_home_section:
            print("Info: 'Smart Home' section not found in Dashy config, creating...")
            smart_home_section = {
                'name': 'Smart Home',
                'icon': 'fa-solid fa-house',
                'items': []
            }
            if 'sections' not in dashy_data:
                dashy_data['sections'] = []
            dashy_data['sections'].append(smart_home_section)
        
        # Add Frigate UI tile if not already present
        frigate_ui_tile_exists = any(item.get('title') == 'Frigate NVR' for item in smart_home_section.get('items', []))
        if not frigate_ui_tile_exists:
            print("Info: Adding Frigate NVR main UI tile to Dashy.")
            smart_home_section['items'].append({
                'title': 'Frigate NVR',
                'description': 'Lokale AI NVR',
                'icon': 'fas fa-video',
                'url': f"http://{domain}:5000",
                'statusCheck': True
            })
        
        # Add individual camera tiles
        for camera_name_key in new_camera_configs.keys():
            tile_exists = any(item.get('title') == f"Frigate Camera: {camera_name_key}" for item in smart_home_section.get('items', []))
            
            if not tile_exists:
                print(f"Info: Adding tile for camera '{camera_name_key}' to Dashy.")
                smart_home_section['items'].append({
                    'title': f"Frigate Camera: {camera_name_key}",
                    'description': f"Live view of {camera_name_key}",
                    'icon': 'fas fa-camera',
                    'url': f"http://{domain}:5000/cameras/{camera_name_key}", 
                    'statusCheck': True 
                })
            else:
                print(f"Info: Tile for camera '{camera_name_key}' already exists in Dashy. Skipping.")

        with open(DASHY_CONFIG_PATH, 'w') as f:
            yaml.dump(dashy_data, f, indent=2, sort_keys=False) 
        print(f"✅ Dashy config.yml successfully updated at: {DASHY_CONFIG_PATH}")

    except yaml.YAMLError as e:
        print(f"❌ Error parsing or writing Dashy config ({e}). Cannot update Dashy tiles.")
    except Exception as e:
        print(f"❌ Unexpected error updating Dashy config: {e}")


async def main():
    """Main function to run the Frigate configuration tool."""
    ...
    should_enter_manual = False # <--- ADDED THIS LINE FOR ROBUSTNESS
    ...
    if should_enter_manual:
        ...
        while True:
            ...
            encoded_username = url_encode_password(username)
    ...
    try:
        ...
        print(f"  bash {BASE_DIR}/scripts/restart-all.sh")
    except Exception as e:
        print(f"❌ Error writing Frigate config.yml: {e}")

    update_dashy_config(all_camera_configs_to_add, domain)
