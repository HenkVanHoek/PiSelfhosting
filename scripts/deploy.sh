...
COMPONENTS_LIST_FILE="$SCRIPTS_DIR/components_list.txt"
SELECTED_COMPONENTS_FILE="$SCRIPTS_DIR/selected_components.txt"
NETWORK_NAME="piselfhosting_net" # Define network name here for consistency
...
init_config_file() {
    local app_name="$1"
    local relative_config_path="$2" # e.g., "config/mosquitto.conf" or "initdb.d/init.sql"
    local source_template="$TEMPLATES_DIR/$app_name/$relative_config_path.template" # Always look for .template
    local dest_path="$DOCKER_COMPOSE_DIR/$app_name/$relative_config_path"

    # Check if the .template source exists
    if [ ! -f "$source_template" ]; then
        echo "❌ Template source not found for $app_name at $source_template. Skipping file creation."
        # This might be expected for services that generate content via scripts, like docker-monitor
        return 1
    fi
    ...
    envsubst < "$source_template" | sudo tee "$dest_path" > /dev/null
    if [ $? -eq 0 ]; then
...
perform_component_specific_actions() {
    ...
    case "$app_name" in
        ...
        "docker-monitor") # Specific actions for docker-monitor, like creating its HTML directory
            ...
            ;;
        "piselfhosting-docs") # Specific actions for piselfhosting-docs, like creating its HTML directory
            sudo mkdir -p "$component_dir/html"
            echo "Info: PiSelfhosting Docs HTML directory checked/created for content."
            ;;
    esac
}
...
generate_helper_scripts() {
    ...
    local DOCKER_COMPOSE_CMD_STRING=$(get_docker_compose_cmd)
    if [ -z "$DOCKER_COMPOSE_CMD_STRING" ]; then
        echo "❌ Docker Compose not found. Cannot generate helper scripts dependent on it."
        return 1
    fi

    # --- Generate start-all.sh ---
    cat <<EOF | sudo tee "$SCRIPTS_DIR/start-all.sh" > /dev/null
#!/bin/bash
...
BASE_DIR="/home/PiSelfhosting"
ENV_FILE="\$BASE_DIR/.env"
NETWORK_NAME="${NETWORK_NAME}" # Use the NETWORK_NAME variable from deploy.sh
...
EOF
    ...
    cat <<EOF | sudo tee "$SCRIPTS_DIR/stop-all.sh" > /dev/null
#!/bin/bash
...
EOF
    ...
    # This script will simply call stop-all then start-all
    cat <<EOF | sudo tee "$SCRIPTS_DIR/restart-all.sh" > /dev/null
#!/bin/bash
...
EOF
    ...
    cat <<EOF | sudo tee "$SCRIPTS_DIR/remove-component.sh" > /dev/null
#!/bin/bash
...
EOF
    ...
    # This script will be called by other helper scripts to determine the correct
    # Docker Compose command (docker compose or docker-compose).
    cat <<EOF | sudo tee "$SCRIPTS_DIR/get_docker_compose_cmd.sh" > /dev/null
#!/bin/bash
...
EOF
    ...
    sudo chmod +x "$SCRIPTS_DIR/start-all.sh" "$SCRIPTS_DIR/stop-all.sh" "$SCRIPTS_DIR/restart-all.sh" "$SCRIPTS_DIR/remove-component.sh" "$SCRIPTS_DIR/get_docker_compose_cmd.sh"
    echo "✅ Helper scripts (start-all, stop-all, restart-all, remove-component, get_docker_compose_cmd) generated."
}
...
# CORRECTED: Changed declare -a to just variable assignment as this is in main script body
COMPONENTS_ORDER=()
...
# The TEMP_VOLUMES_OUT_FILE is an AWK variable, not a shell variable passed via ENVIRON
# The shell variable used to pass its name to AWK needs to be defined here.
...
for comp in "${COMPONENTS_ORDER[@]}"; do
    ...
    if [ "$is_selected" = true ]; then
        ...
        # Special handling for Docker Monitor. It doesn't use a docker-compose.yml.template
        # but its service block is hardcoded here and its HTML is generated by a separate script.
        if [ "$comp" == "docker-monitor" ]; then
            echo "  Generating Docker Compose service block for docker-monitor..."
            # Assuming the HTML will be in docker/docker-monitor/html
            {
                echo "  docker-monitor:"
                echo "    container_name: piselfhosting-docker-monitor"
                echo "    image: nginx:alpine" # Assuming this is the image used for Docker Monitor
                echo "    restart: unless-stopped"
                echo "    volumes:"
                echo "      - ./docker-monitor/html:/usr/share/nginx/html:ro" # Mount the generated HTML
                echo "    ports:"
                echo "      - \"8088:80\"" # Standard port for Docker Monitor
                echo "    networks:"
                echo "      - piselfhosting_net"
            } | tee -a "$TEMP_SERVICES_FILE" > /dev/null # FIXED: Removed 'sudo' here (this was the original bug)
            echo "✅ 'docker-monitor' service definition prepared."
            continue # Skip general template processing for this special case
        fi

        # Process other components that use standard docker-compose.yml.template files
        ...
        compose_template="$TEMPLATES_DIR/$comp/docker-compose.yml.template"
        if [ -f "$compose_template" ]; then
            export HOST_IP=$(hostname -I | awk '{print $1}' | tr -d '[:space:]')

            # Temporary file for envsubst output
            temp_processed_compose=$(mktemp)
            envsubst < "$compose_template" > "$temp_processed_compose"

            # Use awk to correctly parse and reformat services and volumes.
            # Pass TEMP_VOLUMES_OUT_FILE as an AWK variable.
            awk -v service_indent="  " -v temp_volumes_out_file="$TEMP_VOLUMES_OUT_FILE" '
            /^(services|volumes|networks|version):$/ {
                if ($1 == "services:") { in_service=1; in_volume=0; next }
                if ($1 == "volumes:")  { in_volume=1; in_service=0; next }
                if ($1 == "networks:") { in_service=0; in_volume=0; next } # ignore networks in component templates
                if ($1 == "version:")  { in_service=0; in_volume=0; next } # ignore version in component templates
            }
            {
                if (in_service) {
                    gsub(/^[ \t]+/, "", $0) # Remove all leading spaces/tabs
                    print service_indent $0
                } else if (in_volume) {
                    gsub(/^[ \t]+/, "", $0) # Remove all leading spaces/tabs
                    if ($0 ~ /^[a-zA-Z0-9_-]+:$/ || $0 ~ /^[a-zA-Z0-9_-]+:[ \t]*\{.*\}$/ || $0 ~ /^[a-zA-Z0-9_-]+:[ \t]*$/) {
                        print "  " $0 >> temp_volumes_out_file
                    } else {
                        print "    " $0 >> temp_volumes_out_file
                    }
                }
            }
            ' "$temp_processed_compose" >> "$TEMP_SERVICES_FILE"
            rm "$temp_processed_compose" # Clean up temporary file
            echo "✅ '$comp' service definition prepared."
        else
...
if [ -s "$TEMP_VOLUMES_OUT_FILE" ]; then # Check if the temporary volumes file has content
    ...
    # Sort and uniq the collected volume definitions
    sort -u "$TEMP_VOLUMES_OUT_FILE" | while IFS= read -r line; do
        # Append with correct indentation for top-level volumes, which was already added by awk
        echo "${line}" | sudo tee -a "$BASE_DIR/docker-compose.yml" > /dev/null
    done
fi
...
# Added these instructions as they were moved from the end of start-all.sh
