...
for comp in "${COMPONENTS_ORDER[@]}"; do
    ...
    if [ "$is_selected" = true ]; then
        ...
        echo "  $comp:" | tee -a "$TEMP_SERVICES_FILE" > /dev/null # Print service name with correct indent
        ...
        if [ "$comp" == "docker-monitor" ]; then
            ...
            } | tee -a "$TEMP_SERVICES_FILE" > /dev/null # Content goes to temp file
        ...
        if [ -f "$compose_template" ]; then
            ...
            # AWK to extract service CONTENT (everything AFTER "services:" and its direct children)
            # and volumes. Ensure correct indentation for final output.
# AWK to extract service CONTENT (everything AFTER "services:" and its direct children)
            # and volumes. Ensure correct indentation for final output.
            awk '/^services:/ { in_services_block=1; next }
                 in_services_block && /^[[:space:]]*[a-zA-Z0-9_-]+:[[:space:]]*$/ {
                     # This is a service name itself (e.g., `  dashy:`) - skip it as we already printed $comp:
                     in_service_content=1;
                     next
                 }
                 in_services_block && in_service_content {
                     # These are lines *under* the service name. Remove their leading indentation from template
                     # and then add 4 spaces for the final unified compose.
                     gsub(/^[ \t]+/, ""); # Remove all leading spaces
                     print "    " <span class="math-inline">0; \# Add 4 spaces to align under service name
next
\}
/^\(volumes\|networks\)\:</span>/ {
                     in_services_block=0; in_service_content=0; # End of services block
                     if ($1 == "volumes:") { in_volume=1; next } # Start of volumes block
                     if ($1 == "networks:") { in_networks=1; next } # Start of networks block
                 }
                 in_volume { # Handle volumes block content
                     gsub(/^[ \t]+/, "", $0); # Remove all leading spaces/tabs
                     if (<span class="math-inline">0 \~ /^\[a\-zA\-Z0\-9\_\-\]\+\:</span>/ || <span class="math-inline">0 \~ /^\[a\-zA\-Z0\-9\_\-\]\+\:\[ \\t\]\*\\\{\.\*\\\}</span>/ || <span class="math-inline">0 \~ /^\[a\-zA\-Z0\-9\_\-\]\+\:\[ \\t\]\*</span>/) {
                         print "  " $0 >> temp_volumes_out_file; # Top-level volume name
                     } else {
                         print "    " $0 >> temp_volumes_out_file; # Nested volume config
                     }
                 }
                 in_networks { # Ignore networks block content as is handled globally
                    next
                 }
...
if [ -s "$TEMP_VOLUMES_OUT_FILE" ]; then # Check if the temporary volumes file has content
    ...
    sort -u "<span class="math-inline">TEMP\_VOLUMES\_OUT\_FILE" \| while IFS\= read \-r line; do
\# Append with correct indentation for top\-level volumes, which was already added by awk
echo "</span>{line}" | sudo tee -a "$BASE_DIR/docker-compose.yml" > /dev/null
    done
fi
