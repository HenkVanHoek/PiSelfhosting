...
Gebruik deze skill om automatisch LXC-containers aan te maken op je Proxmox VE-server (192.168.178.51) die specifiek zijn ingericht voor PiSelfhosting (inclusief Docker en de benodigde netwerkverbindingen).
...
2. Haalt de openbare SSH-sleutel van PiSelfhosting op.
...
6. Maakt verbinding via SSH en installeert automatisch Docker, en start het `piselfhosting_net` Docker-netwerk.
...
Je kunt de container direct gebruiken als deployment-target in PiSelfhosting door de host op te geven in de configurator of editor-app!
