...
import setup as pisetup  # noqa: E402
...
def mock_project_structure(tmp_path):
    ...
    # Create a dummy template for another service to test merging
    frigate_template_dir = templates_dir / "frigate"
    frigate_template_dir.mkdir()
    (frigate_template_dir / "docker-compose.template.yml").write_text(
        (
            "services:\n  frigate:\n    image: "
    ...
    (project_root / COMPONENTS_METADATA_FILENAME).write_text(
        json.dumps(metadata_content, indent=2)
    )

    # Create selected_components.txt
    (project_root / SELECTED_COMPONENTS_FILENAME).write_text("dashy frigate")
...
def test_load_component_metadata_file_not_found(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        pisetup.load_component_metadata(
            file_path=str(tmp_path / "non_existent_metadata.json")
        )
...
def test_generate_docker_compose_files_and_merge(mock_project_structure):
    """
    Tests that individual compose files are generated and then merged
    into a single unified docker-compose.yml.
    """
    # Load the mock data
    metadata = pisetup.load_component_metadata()
    selected_components = pisetup.read_selected_components()

    # Run the generation function
    pisetup.generate_docker_compose_files(
        metadata["all_component_data"], selected_components
    )

    # Check that the unified file was created
    output_dir = mock_project_structure / DOCKER_COMPOSE_OUTPUT_DIR
    unified_compose_file = output_dir / UNIFIED_DOCKER_COMPOSE_FILENAME
    assert unified_compose_file.exists()

    # Load the generated YAML and check its contents
    with open(unified_compose_file, "r") as f:
        data = yaml.safe_load(f)

    # Check for services from both selected components
    assert "services" in data
    assert "dashy" in data["services"]
    assert "frigate" in data["services"]
    assert data["services"]["dashy"]["image"] == "lissy93/dashy"
    assert (
        data["services"]["frigate"]["image"] == "ghcr.io/blakeblackshear/frigate:stable"
    )

    # Check for volumes
    assert "volumes" in data
    assert "frigate_data" in data["volumes"]
