...
def main(host_ip):
    ...
    if not dashy_config['sections'] or 'items' not in dashy_config['sections'][0]:
        dashy_config['sections'] = [{'name': 'My Services', 'icon': 'fas fa-rocket', 'items': []}]

    # Get a list of existing tile titles to prevent duplicates
    existing_titles = {item.get('title', '') for item in dashy_config['sections'][0]['items']}
    ...
    for component_name in selected_components:
        ...
        if comp_meta and comp_meta.get('has_ui') and comp_meta.get('name') not in existing_titles:
            print(f"Found new component with UI: {comp_meta['name']}")

            new_tile = {
                'title': comp_meta['name'],
                'icon': comp_meta.get('icon', 'fas fa-server'),
                'url': f"{comp_meta.get('protocol', 'http')}://{host_ip}:{comp_meta.get('ui_port')}"
            }
