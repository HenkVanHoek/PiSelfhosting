...
class ComponentManager:
    ...
    def _enrich_components_with_variables(self):
        """
        Scans for variables.json files and merges them into the loaded
        metadata, establishing variables.json as the Single Source of Truth.
        """
        components = self._components_data.get("components", {})
        for comp_id, comp_details in components.items():
            if not comp_details.get("has_configuration"):
                continue
            ...
            if variables_file.exists():
                try:
                    with open(variables_file, "r", encoding="utf-8") as f:
                        data = json.load(f)
                        comp_details["required_variables"] = data.get("variables", [])
                except (json.JSONDecodeError, TypeError) as e:
                    logger.error(f"Error processing variables for {comp_id}: {e}")
                    comp_details["required_variables"] = []
    ...
    def validate_component_configuration(
        component_id: str, template_content: str, variables: List[Dict[str, Any]]
    ):
        """
        ...
        # --- DEFINITIVE FIX: Add Rule 3 ---
        # Rule 3: All defined variables must be used in the template.
    ...
    def update_component_variables(
        ...
        if component_id in self._components_data.get("components", {}):
            self._components_data["components"][component_id]["required_variables"] = (
                variables_data.get("variables", [])
            )
