...
class SyncManager:
    ...
    def fetch_from_remote(self) -> bool:
        ...
        repo = os.environ.get(
            ...
            "HenkVanHoek/piselfhosting-components",
        )
    ...
    def get_sync_status(self) -> dict:
        ...
        if remote_fetched:
            ...
            local_rules = local_data.get("_piselfhosting", {}).get("group_rules", {})
            remote_rules = remote_data.get("_piselfhosting", {}).get("group_rules", {})

            local_order = local_data.get("_piselfhosting", {}).get("group_order", [])
            remote_order = remote_data.get("_piselfhosting", {}).get("group_order", [])
    ...
    def check_write_access(self) -> bool:
        ...
        https_url = "https://github.com/HenkVanHoek/piselfhosting-components.git"
        try:
            res = subprocess.run(  # nosec B603 B607
