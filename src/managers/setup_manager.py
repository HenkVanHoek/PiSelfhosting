...
class SetupManager:
    ...
    def __init__(self, component_manager, output_dir=None,
        ...
        if template_base_path:
            ...
            self.template_base_path = Path(
                __file__).parent.parent.parent / "component_templates"

        self.docker_compose_path = self.output_dir / "docker-compose.yml"

    def generate_all_files(self, selected_components, user_variables,
                           managed_devices):
        errors = []
        logger.info(
        ...
        if managed_devices:
            ...
            auto_vars["PISelfhosting_HOST_IP"] = managed_devices[0].get("ip",
                                                                     "127.0.0.1")
        else:
        ...
        try:
            full_component_list = self._resolve_dependencies(
                selected_components)
        ...
        for component_id in full_component_list:
            ...
            if not details:
                errors.append(
                    f"Component '{component_id}' not found in metadata.")
                continue
            ...
            try:
                ...
                if "other_files" in details:
                    self._generate_other_files(component_id, details, jinja_env,
                                               component_render_context, errors)
            ...
            try:
                ...
                with open(self.docker_compose_path, "w") as f:
                    yaml.dump(compose_data, f, Dumper=DockerComposeYAMLDumper,
                              ...
                              width=float("inf"))
                logger.info(
                    f"Successfully generated {self.docker_compose_path}")
            except IOError as e:
                errors.append(f"Failed to write docker-compose file: {e}")

        return not errors, errors

    def _resolve_dependencies(self, selected_components: list[str]) -> list[
        str]:
        resolved = []
        visited = set()
        ...
        for initial_comp_id in selected_components:
            ...
            while stack:
                ...
                unresolved_dependencies = [dep for dep in dependencies if
                                           dep not in visited]
                ...
                if not unresolved_dependencies:
                    ...
                    for dep_id in unresolved_dependencies:
                        if dep_id in visiting:
                            raise ValueError(
                                f"Circular dependency detected involving '{dep_id}' and '{comp_id}'")
                        stack.append(dep_id)
                        visiting.add(dep_id)

        return self.component_manager.sort_components_by_master_order(resolved)

    def _merge_docker_compose_template(self, comp_id, jinja_env, compose_data,
                                       context, errors):
        try:
    ...
    def _generate_other_files(self, component_id, details, jinja_env, context,
                              errors):
