        ...
        for device in managed_devices:
            ...
            try:
                ...
                exit_code, remote_home_dir = ssh.execute_command(
                    "echo $HOME", dummy_callback
                )
                if exit_code != 0 or not remote_home_dir:
                    log_step("FATAL ERROR: Could not determine remote home directory.")
                    overall_success = False
                    continue
                log_step(f"SUCCESS: Remote home directory is '{remote_home_dir}'")
                    ...
                    with tarfile.open(tarball_path, "w:gz") as tar:
                        for item in os.listdir(local_output_path):
                            if item != "deployment.tar.gz":
                                tar.add(
                                    os.path.join(local_output_path, item), arcname=item
                                )
                    log_step("SUCCESS: Local deployment archive created.")
                except Exception as e:
                    log_step(f"FATAL ERROR: Could not create local tarball: {e}")
                    overall_success = False
                    continue

                log_step(f"Uploading deployment archive to {remote_tmp_tarball}...")
                try:
                    with open(tarball_path, "rb") as f:
                        content = f.read()
                    uploaded, msg = ssh.upload_content(content, remote_tmp_tarball)
                    if not uploaded:
                ...
                log_step(
                    "Ensuring remote deployment directory"
                    f" '{remote_deployment_dir}' exists..."
                )
                ...
                extract_command = (
                    f"tar -xzf {remote_tmp_tarball} -C {remote_deployment_dir}"
                )
                ...
                if overall_success:
                    log_step("Discovering web interfaces for deployed services...")
                    try:
                        compose_path = Path(output_path) / "docker-compose.yml"
                        with open(compose_path, "rb") as f:
                            compose_data = yaml.safe_load(f.read().decode("utf-8"))

                        service_links = []
                        all_components = (
                            self.component_manager.get_all_components_dict()
                        )

                        for service_name, service_config in compose_data.get(
                            "services", {}
                        ).items():
                            component_id = service_name.replace("piselfhosting-", "", 1)
                        ...
                        log_step(f"WARN: Could not discover web interfaces: {e}")
